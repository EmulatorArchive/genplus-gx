...
/* improved BUSREQ mechanism accuracy (EkeEke)*/
...
void gen_reset (unsigned int hard_reset)
{
	if (hard_reset)
	{
		...
		/* Reset ROM Banking */
		int i;
		rom_readmap[0] = (bios_enabled == 3) ? &genrom.bios_rom[0] : &genrom.cart_rom[0];
		for (i=1; i<8; i++) rom_readmap[i] = &genrom.cart_rom[i << 19];
		rom_size = (bios_enabled == 3) ? 0x800 : genrom.genromsize;
	}
