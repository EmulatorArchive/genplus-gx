...
void getrominfo (char *romheader)
{
  ...
  realchecksum = GetRealChecksum (((uint8 *) genrom.cart_rom) + 0x200, genrom.genromsize - 0x200);
#ifdef LSB_FIRST
...
int load_rom(char *filename)
{
...
#ifdef NGC
	size = genrom.genromsize;
#else
	uint8 *ptr;
	ptr = load_archive(filename, &size);
    if(!ptr) return (0);
    memcpy(genrom.cart_rom, ptr + offset, size);
    free(ptr);
#endif

	/* detect interleaved roms (.smd format) */
	if (strncmp((char *)(genrom.cart_rom + 0x100),"SEGA", 4) && ((size / 512) & 1))
	{
		size -= 512;
		offset += 512;
		...
		{
			deinterleave_block (genrom.cart_rom + offset + (i * 0x4000));
		}

	    memcpy(genrom.cart_rom, genrom.cart_rom + offset, size);
	}

	/* max. 5 MBytes supported */
	if (size > GENESIS_CART_ROM_SIZE) size = GENESIS_CART_ROM_SIZE;
	genrom.genromsize = size;
	
	/* clear unused ROM space */
	if (size < GENESIS_CART_ROM_SIZE) memset (genrom.cart_rom + size, 0x00, GENESIS_CART_ROM_SIZE - size);

	getrominfo((char *)genrom.cart_rom);	/* get infos from ROM header */
	...
	/* Byteswap ROM */
	uint8 temp;
	for(i = 0; i < genrom.genromsize; i += 2)
	{
		temp = genrom.cart_rom[i];
		cart_rom[i] = genrom.cart_rom[i+1];
		cart_rom[i+1] = temp;
	}
#endif
	    ...
	    ((strstr(rominfo.product,"-K0109") != NULL) && (rominfo.checksum == 0x4f10)))
	{
		...
		{
			temp = genrom.cart_rom[i];
			genrom.cart_rom[i] = genrom.cart_rom[i+1];
			genrom.cart_rom[i+1] = temp;
		}
	}
