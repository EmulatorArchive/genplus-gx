...
void sram_init()
{
  ...
  if ((genrom.cart_rom[0x1b0] == 0x52) && (genrom.cart_rom[0x1b1] == 0x41))
  {
  	  /* retrieve informations from headezr */
	  sram.detected = 1;
      sram.start = READ_WORD_LONG(genrom.cart_rom, 0x1b4);
      sram.end   = READ_WORD_LONG(genrom.cart_rom, 0x1b8);
  ...
  if (genrom.genromsize <= sram.start)
  {
	  sram.on = 1;
	  sram.write = 1;
  }
