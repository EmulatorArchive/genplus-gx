...
/* DMA Timings
 ...
 NOTE: These transfer rates are actually derivated from VDP access timings,
	   ...
	   VRAM access are BYTE access (transfer rate divided by 2) 
	   VSRAM/CRAM access are WORD access

	   H32 VBLANK Timings are adjusted in the case of 68K to VRAM DMA
	   Kawasaki Superbike Challenge & Formula One need 84 bytes
*/
...
static inline void dma_fill(unsigned int data)
{
	...
	if ((addr & sat_base_mask) == satb)
	{
		do
		{
			/* update internal SAT (fixes Battletech) */
...
{
	uint8 hc = (hc_latch == -1) ? hctab[count_m68k - line_m68k] : (hc_latch & 0xFF);
