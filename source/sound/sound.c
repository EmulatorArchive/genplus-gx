...
unsigned int Sound_Extrapol[313][2];
...
void sound_init(int rate)
{
	...
	if (FM_GENS)
	{
		...
		for(i = 0; i < 313; i++)
		{
			Sound_Extrapol[i][0] = ((snd.buffer_size * i) / lines_per_frame);
			Sound_Extrapol[i][1] = (((snd.buffer_size * (i + 1)) / lines_per_frame) - Sound_Extrapol[i][0]);
		}
