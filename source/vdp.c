...
void dma_vbus (void)
{
  ...
  if ((code & 0x0F) == 0x01) dma_update(0,length); // copy to VRAM
  else dma_update(1,length);                       // copy to VSRAM or CRAM
...
void dma_fill(uint16 data)
{
	...
	dma_update(2, length);
	WRITE_BYTE (vram, (addr & 0xFFFF)^1, data & 0xFF);

    do
	{
		WRITE_BYTE (vram, (addr & 0xFFFF), (data >> 8) & 0xFF);
	    MARK_BG_DIRTY (addr);
	    addr += reg[15];
	}
    while (--length);
