...
#define Swap32(x) (((x & 0xFFFF0000) >> 16) | ((x & 0x0000FFFF) << 16))
...
WRITE_LONG (void *address, uint32 data)
{
  if ((uint32) address & 3)
    {
#ifdef LSB_FIRST
...
#define DRAW_SPRITE_TILE \
    if (lb[0] || lb[1] || lb[2] || lb[3] || lb[4] || lb[5] || lb[6] || lb[7]) status |= 0x20; \
...
void render_line(int line)
{
    ...
    else
    {
        ...
        if(im2_flag) render_ntw_im2(line, nta_buf);
        else  render_ntw(line, nta_buf);
...
void render_ntx(int which, int line, uint8 *buf)
{
    ...
    int vsr_shift;
    uint32 *vs;
    uint16 table = (which) ? ntbb : ntab;
	int xshift   = (which) ? 2 : 0;

    get_hscroll(line, xshift, &xscroll);
    shift = (xscroll & 0x0F);
...
// nta column starts
    ...
    y_scroll = (Swap32 (vs[0]) >> vsr_shift) & 0x3FF;
...
void render_ntx_im2(int which, int line, uint8 *buf)
{
    ...
    y_scroll = (Swap32 (vs[0]) >> vsr_shift) & 0xFFFF;
...
void render_ntx_vs(int which, int line, uint8 *buf)
{
    ...
    for(column = start; column < end; column ++, index ++)
    {
        y_scroll = (Swap32 (vs[column]) >> vsr_shift) & 0x3FF;
        y_scroll = (line + y_scroll) & playfield_row_mask;
        v_line = (y_scroll & 7) << 3;
        nt = (uint32 *)&vram[table + (((y_scroll >> 3) << playfield_shift) & y_mask)];
        atbuf = nt[index & playfield_col_mask];
...
//          object_info[object_index_count].xpos = *(uint16 *)&q[6];
	        ...
	        object_info[object_index_count].size = q[3 ^ 1];
