 ...
 */
 ...
 void patch_game()
 {
	 if ((strstr(rominfo.domestic,"GALAHAD") != NULL)        ||
		 (strstr(rominfo.international,"GALAHAD") != NULL)   ||
		 (strstr(rominfo.domestic,"ROAD RASH") != NULL)      ||
		 (strstr(rominfo.international,"ROAD RASH") != NULL) ||
         (strstr(rominfo.domestic,"Road Rash") != NULL)      ||
		 (strstr(rominfo.international,"Road Rash") != NULL)
		) alttiming = 1;
	 else alttiming = 0;

	 if ((strstr(rominfo.domestic,"CHAOS ENGINE") != NULL) ||
		 (strstr(rominfo.international,"CHAOS ENGINE") != NULL) ||
		 (strstr(rominfo.domestic,"SOLDIERS OF FORTUNE") != NULL) ||
		 (strstr(rominfo.international,"SOLDIERS OF FORTUNE") != NULL)
		) vdptiming = 1;
	 else vdptiming = 0;

	 if ((strstr(rominfo.domestic,"Sesame Street") != NULL) ||
		 (strstr(rominfo.international,"Sesame Street") != NULL)
		) irqtiming = 1;
     else irqtiming = 0;

	 if ((strstr(rominfo.domestic,"MENACER") != NULL) ||
		 (strstr(rominfo.international,"MENACER") != NULL))
	 {
...
void getrominfo (char *romheader)
{
  ...
  patch_game(); /* patch specific games */
}
