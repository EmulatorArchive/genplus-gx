...
void io_reset(void)
{
    ...
    switch (input.system[0])
    {
		...
		case SYSTEM_TEAMPLAYER:
			port[0].data_w = multitap_1_write;  
			port[0].data_r = multitap_1_read;
			break;
    ...
    switch (input.system[1])
    {
		...
		case SYSTEM_WAYPLAY:
		case SYSTEM_TEAMPLAYER:
			port[1].data_w = multitap_2_write;  
			port[1].data_r = multitap_2_read;
			break;
...
unsigned int io_read(unsigned int offset)
{
    switch(offset)
    {
        case 0x00: /* Version register */
		{
			...
			uint8 gen_ver = (bios_enabled == 3) ? 0x01 : 0x00; /* hardware version */
