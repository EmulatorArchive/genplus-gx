...
uint8 bios_enabled;
uint32 rom_size;
uint8 rom_write;		  /* enable ROM writes (Game no Kanzume Otokuyou) */
uint8 *rom_readmap[8];
uint16 cpu_sync[512];
...
/* ROM Bankswitch mechanism
   ...
   Bank 0 is fixed
 */
void rom_bank_w (int address, int value)
{
	...
	switch (bank)
	{
		case 0:
			...
			/* custom ROM banskwitch (Game no Kanzume Otokuyou) */
			if (value == 0xF8)  rom_write = 1;
...
/* alternate ROM Bankswitch mechanism
   used in Multi-in-1 pirate cartridge 
   (credits to Notaz/Haze)
 */
void multi_bank_w (int address)
{
	uint32 bank_addr = (address & 0x3F) << 16;
	if (bank_addr < genromsize) rom_readmap[0] = &cart_rom[bank_addr];
}
