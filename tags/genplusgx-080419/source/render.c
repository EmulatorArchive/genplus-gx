...
uint8 rgb565_norm[2][8] = {{0 ,  5,  9, 13, 18, 22, 27, 31},
					       {0 ,  9, 18, 27, 36, 45, 54, 63}};
uint8 rgb565_half[2][8] = {{0 ,  2,  4,  6,  8, 10, 12, 15},
						   {0 ,  4,  9, 13, 18, 22, 27, 31}};
uint8 rgb565_high[2][8] = {{15, 18, 20, 22, 24, 26, 28, 31},
						   {31, 36, 40, 44, 49, 53, 58, 63}};
...
void render_line(int line, uint8 odd_frame)
{
	...
	if (use_480i && interlaced) vline = (vline * 2) + odd_frame;	/* double-resolution mode */
	...
	else
    {
		...
		/* SMS feature: which game uses this ? */
		/*if(reg[0] & 0x20) memset(&lb[0x20], 0x40 | border, 0x08);*/
    }
