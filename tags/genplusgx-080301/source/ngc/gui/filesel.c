...
int updateSDdirname()
{
  ...
  char temp[1024];
   ...
   else if (strcmp(filelist[selection].filename,"..") == 0) 
   {
	 ...
	 /* handles root name */
	 if (strcmp(rootSDdir,sdslot ?  "dev1:":"dev0:") == 0) sprintf(rootSDdir,"dev%d:\\genplus\\..",sdslot);
   ...
   else
   {
     ...
     {
       /* handles root name */
	   if (strcmp(rootSDdir,sdslot ? "dev1:\\genplus\\..":"dev0:\\genplus\\..") == 0) sprintf(rootSDdir,"dev%d:",sdslot);
	...
	while (haverom == 0)
	{
		...
		if (p & PAD_BUTTON_B)
		{
			...
			if (((!UseSDCARD) && (basedir == rootdir)) ||
				(UseSDCARD && strcmp(rootSDdir,sdslot?"dev1:\\genplus\\..":"dev0:\\genplus\\..") == 0)) return;
			go_up = 1;
		}
...
int OpenSD (uint8 slot) 
{
  ...
  if (haveSDdir == 0)
  {
     ...
     sprintf(rootSDdir,"dev%d:\\genplus\\roms", slot);
	 sdslot = slot;
